...
let src = './assets/Azusa_home/Azusa_home';
...
if (ret && ret[1]['V4'] === true) {
    ...
    import("./player_legacy.cjs").then(PlayerLegacy => {
        Canvas = new PlayerLegacy.PlayerLegacy(param, src, ret[1]['format'] ? '.json' : '.skel', ret[1]['injectPlayerSettings']);
        Canvas.play().then((res) => {window.postMessage(JSON.stringify(res, null, 4), '*')});
    });
}
